...
class sessman {
  ...
  private function auth_db($username, $secret) {
    ...
    // Determine session state
    $user = $this->db->query(
      "SELECT id, username, firstname, lastname, tokens, sa FROM sessman_user WHERE username=:username AND $dbqsecret",
  ...
  function user_list($id = null) {
    ...
    if ($id == null) {
      ...
      return $this->db->query('SELECT id, username, firstname, lastname, tokens, sa FROM sessman_user ORDER BY username', []);
    }
    else {
      // List 'self'
      if ($id == 'self') {
    ...
    // Prepare response
    $list = $this->db->query('SELECT id, username, firstname, lastname, tokens, sa FROM sessman_user WHERE id=:id', ['id'=>$id]); 
    if ($self) {
      unset($list[0]->id);
    }
    return $list;
  ...
  function user_save($id, $data) {
    if ($id == 'self') {
      ...
      if (!$this->SA()) { return false; }
      $fieldlist = ['username', 'password', 'firstname', 'lastname', 'tokens', 'sa'];
    }
