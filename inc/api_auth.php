...
$sessman = new sessman($db, "php-RestAPI_session");
...
if (!$sessman->authorized())  { $api->http_error(403); }
...
if ($api->route('/auth/user/{user}/token')) {
  ...
  if ($api->method("POST"))     { 
    $token = $sessman->usertoken_save($api->param('user'), null, $payload['name'], $payload['expiry']);
    if ($token != false) {
      $result = ['token'=>$token];
    }
  }
